...
function changeLife(player, amount) {
  if (player === 1) life1 = Math.max(life1 + amount, 0);
  else life2 = Math.max(life2 + amount, 0);
  updateDisplay('life', player);
  sendState();
  removeHighlight();
}
function changeAP(player, amount) {
  if (player === 1) ap1 = Math.max(ap1 + amount, 0);
  else ap2 = Math.max(ap2 + amount, 0);
  updateDisplay('ap', player);
  sendState();
  removeHighlight();
}
function changePower(player, amount) {
  if (player === 1) power1 = Math.max(power1 + amount, 0);
  else power2 = Math.max(power2 + amount, 0);
  updateDisplay('power', player);
  sendState();
  removeHighlight();
}
...
setInterval(() => {
  let nextIndex;
  do { nextIndex = Math.floor(Math.random() * medias.length); }
...
function highlightFirstPlayer(player) {
  ...
  p1.classList.remove("start-highlight");
  p2.classList.remove("start-highlight");

  if (player === 1) p1.classList.add("start-highlight");
  else p2.classList.add("start-highlight");
}

function removeHighlight() {
  document.getElementById('player1').classList.remove("start-highlight");
  document.getElementById('player2').classList.remove("start-highlight");
}
